...
class DropCopy:
    ...
    def clean(self):
        ...
        self.DC0001_brk_num = int(self.DC0001_brk_num)
...
def thread_func(client_socket, SESSION):
    ...
    while BOOM:
        ...
        try:
            ...
            # Requests for new response or data from server only when their is data length in buffer
            # less then 318 as drop_copy size for fo is 318 , try to collect all the data before processing it
            if len(buffer)>=318:
                response = buffer[:318]
                buffer=buffer[318:]
            ...
            # if their is buffer or data left out from last data processing then attache the remaning data from
            # buffer to new response so that it can be processed
            if buffer:
                response = buffer+response
                buffer=b""

            # extract the length variable from the response by processing the response and only use the 
            # data from response of length = _length ,
            # if response has data longer than _length then dump the chooped data into buffer so that it can be processed with
            # next reponse
            _length = struct.unpack("!h",response[:2])[0]
            if _length!=318:
                # \x01>\x00\x00\x00\x00 is a magic byte for dropcopy response, 
                # now from the response chop out the unnecessary damaged/corrupted data and then
                # store the actual data into buffer to process later 
